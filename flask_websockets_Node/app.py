...
tab1 = html.Div(children=[
    ...
    html.Div(id="appflaskvar",
    children=flaskvar),

    html.Button('Update FV', id='updateflaskvar', n_clicks_timestamp=0),
...
###
@dash_app.callback(Output('memory-tab2', 'value'),
              Input('cytoscapenetw', 'tapNodeData'))
def saveDataIndiv(data):
    global pickednode
    global diseaseselected
    pickednode = data
    diseaseselected = False
...
#callback for updating flask variable
@dash_app.callback(
    Output('appflaskvar', 'children'),
    Input('updateflaskvar',  'n_clicks_timestamp')

)
def update_flaskvar(ns):
    global flaskvar
    print("flaskvar")
    print(flaskvar)
    children = str(flaskvar)
    return children
...
def update_output2(value):
  ...
  #  print(str(pickednode))
   # return 'You have selected 2: \n{}'.format(pickednode)
    ...
    return "individual node" + str(value)
